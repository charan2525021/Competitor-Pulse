    ...
    try {
      ...
      if (Array.isArray(rows) && rows.length > 0) {
        ...
        if (existing.cnt === 0) {
          ...
          tx();
          console.log(`[Store] Imported ${rows.length} items from ${col}.json into SQLite`);
        }
      }
    } catch (e) {
      console.warn(`[Store] Failed to import ${col}.json:`, (e as Error).message);
    }
  }
}
// Import config.json if DB config is empty
...
if (fs.existsSync(cfgPath)) {
  try {
    ...
    if (existing.cnt === 0) {
      ...
      console.log("[Store] Imported config.json into SQLite");
    }
  } catch (e) {
    console.warn("[Store] Failed to import config.json:", (e as Error).message);
  }
}
