...
export function StatCard({ label, value, icon, color = "blue", delay = 0 }: StatCardProps) {
  ...
  return (
    <div
      className="rounded-2xl p-5 animate-scale-in transition-all duration-200 hover:-translate-y-0.5 card-shimmer bg-card border border-border shadow-sm"
    ...
    >
      ...
      <div className="text-xs mt-1 font-medium text-muted-foreground">{label}</div>
    </div>
  );
}
